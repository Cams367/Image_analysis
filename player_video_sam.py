...
def save_all_frames(video_path, output_base_folder, frames_with_masks):
    ...
    output_folder_path = os.path.join(os.path.dirname(video_path), f"{video_name_no_ext}")
    ...
    while True:
        ...
        frame_filename = os.path.join(output_folder_path, f"{frame_idx:06d}.png")
...
def save_segmented_objects_as_video(video_path, frames_with_masks, output_folder_path, codec='mp4v', fps=None):
    ...
    parts=list(path.parts)
    parts[1] = "sam_treated"
    output_folder_path = os.path.join(*parts[:-1])
    
    
    output_video_path = os.path.join(output_folder_path, f"{video_name_no_ext}.mp4")
    ...
    while ret:
        ...
        if key == ord('q') or key == 27: break # Quitter
            ...
            if app_state.propagation_done:
                ...
                video_filename = os.path.basename(video_parent_dir)
                video_name_no_ext, _ = os.path.splitext(video_filename)
                output_folder_path = os.path.join(os.path.dirname(video_path), f"{video_name_no_ext}")
